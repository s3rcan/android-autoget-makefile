...
set timeout -1                                                   \n\
                                                                 \n\
spawn $(ANDROID_SDK_LOCATION)/tools/android update sdk -u -a -t  android-23,android-22,platform-tools,build-tools-25.0.2,addon-google_apis-google-23,extra-android-m2repository,extra-google-m2repository \n\
expect {                                                         \n\
    \"\[y\\/n\]: \" {                                            \n\
        send \"y\\\r\"                                           \n\
        expect \"y\\\r\"                                         \n\
...
$(ANDROID_SDK_LOCATION)/.extracted : download_sdk
	...
	@unzip -q $(ANDROID_SDK_ZIP) -d $(ANDROID_SDK_LOCATION)
	touch $@
...
$(ANDROID_NDK_LOCATION)/.extracted : download_ndk
	@echo Extracting Android SDK
	@unzip -q $(ANDROID_NDK_ZIP) -d $(ANDROID_NDK_LOCATION)
	touch $@
